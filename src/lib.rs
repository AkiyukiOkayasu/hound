    ...
    fn read<R: io::Read>(reader: &mut R, fmt: SampleFormat, bytes: u16, bits: u16) -> Result<Self> {
        match fmt {
            ...
            SampleFormat::Int =>
                ...
                // (actually: 25 bits).
                match (bytes, bits) {
                    (1, 8) => Ok(try!(reader.read_u8().map(signed_from_u8).map(|x| x as f32 / 128.0f32))),
                    (2, 16) => Ok(try!(reader.read_le_i16().map(|x| x as f32 / 32768.0f32))),
                    (3, 24) => Ok(try!(reader.read_le_i24().map(|x| x as f32 / 8388608.0f32))),
